...
public class AsyncDeletionService {
  ...
  public AsyncDeletionService(BlockWorker worker, BlockingQueue queue) {
    ...
    mToAvoidDuplicateBlock = Collections.newSetFromMap(new ConcurrentHashMap<Long, Boolean>());
  ...
  public void pendingBlocksToBeDeleted(List<Long> blocks) {
    for (long id : blocks) {
      if (mToAvoidDuplicateBlock.contains(id)) {
        LOG.info("{} is pending. Current queue size is {}.", id, mPendingRemovedBlocks.size());
        continue;
      }
      try {
        ...
        LOG.warn("AsyncDeletionService got interrupted while it was puting block {}.", id);
      }
    }
  }
