...
public class FileInStream extends InputStream
  ...
  protected FileInStream(URIStatus status, InStreamOptions options, FileSystemContext context) {
    ...
    mContext = context;
    mAlluxioStorageType = options.getOptions().getReadType().getAlluxioStorageType();
    mShouldCache = mAlluxioStorageType.isStore();
    mCachePartiallyReadBlock = true;
    mClosed = false;
    if (mShouldCache) {
      Preconditions.checkNotNull(options.getOptions().getCacheLocationPolicy(),
          PreconditionMessage.FILE_WRITE_LOCATION_POLICY_UNSPECIFIED);
    }

    long bytes = Configuration.getBytes(PropertyKey.USER_FILE_SEEK_BUFFER_SIZE_BYTES);
    int seekBufferSizeBytes = Math.max((int) bytes, 1);
  ...
  private int readInternal() throws IOException {
    ...
    final int data = mCurrentBlockInStream.read();
    if (data == -1) {
    ...
    if (mStatus.isPersisted()) {
      ...
      openUfsBlockOptions =
          Protocol.OpenUfsBlockOptions.newBuilder().setUfsPath(mStatus.getUfsPath())
              ...
              .setMaxUfsReadConcurrency(mInStreamOptions.getOptions().getMaxUfsReadConcurrency())
              .setNoCache(!mInStreamOptions.getOptions().getReadType().getAlluxioStorageType().isStore())
              .setMountId(mStatus.getMountId()).build();
    }
    return mBlockStore.getInStream(blockId, mInStreamOptions, null);
  }
